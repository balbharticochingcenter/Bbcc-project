...
// ================= WHATSAPP SEND SHEET FUNCTIONS =================

// 1. MAIN FUNCTION - Download WhatsApp Send Sheet
function downloadWhatsAppSheet() {
    const rows = document.querySelectorAll("#smsReminderBody tr");
    if (rows.length === 0) {
        alert("Koi data nahi hai download karne ke liye!");
        return;
    ...
    let csv = "Student Mobile,SMS Text,Send WhatsApp,Delete Row\n";
    
    // PART 1: Student numbers
    rows.forEach(row => {
        const studentMobileCell = row.querySelector("td:nth-child(8)");
        const studentMobile = studentMobileCell ? studentMobileCell.innerText.trim() : "";
        const smsText = row.querySelector(".msgBox")?.value.trim() || "";
        ...
        // Format SMS text for single line
        const singleLineText = formatSmsForCSV(smsText);
        
        // Create WhatsApp link
        const whatsappLink = createWhatsAppLink(studentMobile, smsText);
        
        csv += `"${studentMobile}","${singleLineText}","${whatsappLink}","DELETE_${studentMobile}"\n`;
    });
    
    // Add empty row separator
    csv += "\n";
    
    // PART 2: Parent numbers
    rows.forEach(row => {
        const parentMobileCell = row.querySelector("td:nth-child(7)");
        const parentMobile = parentMobileCell ? parentMobileCell.innerText.trim() : "";
        const smsText = row.querySelector(".msgBox")?.value.trim() || "";
        ...
        // Create WhatsApp link
        const whatsappLink = createWhatsAppLink(parentMobile, smsText);
        
        csv += `"${parentMobile}","${singleLineText}","${whatsappLink}","DELETE_${parentMobile}"\n`;
    });
    
    downloadCSV(csv, `whatsapp_send_sheet_${getFormattedDate()}.csv`);
    
    // Show instructions
    showWhatsAppSheetInstructions();
}

// 2. HELPER: Format SMS for CSV
function formatSmsForCSV(text) {
    if (!text) return "";
    
    // Escape quotes
    let formatted = text.replace(/"/g, '""');
    
    // Replace newlines with space (for single line in CSV)
...
// 3. HELPER: Create WhatsApp hyperlink
function createWhatsAppLink(mobile, message) {
    ...
    return `=HYPERLINK("${whatsappURL}", "Send WhatsApp")`;
}

// 4. HELPER: Create Delete button formula
function createDeleteFormula(identifier) {
    // This creates an Excel formula that can be used with VBA/macro
    return `=IF(A2<>"", "DELETE", "")`;
}

// 5. INSTRUCTIONS for user
function showWhatsAppSheetInstructions() {
    const instructions = `
✅ WhatsApp Send Sheet Download Ho Gaya!

📋 EXCEL MEIN KAISE USE KAREIN:

STEP 1: Excel mein file open karein
STEP 2: "Column A" - Mobile Numbers
STEP 3: "Column B" - SMS Text
STEP 4: "Column C" - Send WhatsApp (Green button)
       - Click karein: WhatsApp chat khulega with pre-filled message
STEP 5: "Column D" - Delete Row
       - DELETE likha hua row delete kar sakte hain

📱 WHATSAPP SEND KARNE KA TARIKA:
1. Column C mein "Send WhatsApp" button pe click karein
2. WhatsApp web/desktop khulega
3. Message automatically paste hoga
4. Send button dabayein

🗑️ ROW DELETE KARNE KA TARIKA:
1. Excel ke "Developer" tab mein jayein
2. "Insert" > "Button" select karein
3. Ye macro code paste karein:
----------------------------------------
Sub DeleteMarkedRows()
    Dim r As Long
    For r = Cells(Rows.Count, "D").End(xlUp).Row To 2 Step -1
        If Cells(r, "D").Value = "DELETE" Then
            Rows(r).Delete
        End If
    Next r
End Sub
----------------------------------------
4. Button se macro connect karein
5. "DELETE" likhi rows delete ho jayengi

⚠️ NOTE: 
- Sirf valid mobile numbers CSV mein aaye hain
- N/A ya invalid numbers automatically skip ho gaye hain
`;

    // Show instructions in a modal
    const instructionModal = document.createElement('div');
    instructionModal.id = 'instructionModal';
    ...
    instructionModal.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
            <h3 style="margin: 0; color: #25D366;">📋 Instructions for WhatsApp Sheet</h3>
            <button onclick="closeInstructionModal()" style="background: #e74c3c; color: white; border: none; border-radius: 50%; width: 30px; height: 30px; cursor: pointer;">×</button>
        </div>
        <div style="white-space: pre-line; line-height: 1.5; color: #333;">
            ${instructions}
        </div>
        <div style="margin-top: 20px; text-align: center;">
            <button onclick="closeInstructionModal()" style="background: #25D366; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;">
                ✅ Samajh Gaya
            </button>
        </div>
    `;
...
// 7. ALTERNATIVE: Simple Excel with buttons (No macros)
function downloadSimpleWhatsAppSheet() {
    const rows = document.querySelectorAll("#smsReminderBody tr");
    if (rows.length === 0) {
        alert("Koi data nahi hai!");
        return;
    ...
    let csv = "Mobile Number,SMS Text,WhatsApp Link,Mark for Delete\n";
    
    // Process all valid numbers (both student and parent)
    const allNumbers = [];
    
    rows.forEach(row => {
        const studentMobile = row.querySelector("td:nth-child(8)")?.innerText.trim() || "";
        const parentMobile = row.querySelector("td:nth-child(7)")?.innerText.trim() || "";
        const smsText = row.querySelector(".msgBox")?.value.trim() || "";
        
        // Add student mobile if valid
        if (isValidMobile(studentMobile)) {
            allNumbers.push({
    ...
    allNumbers.forEach(item => {
        const singleLineText = formatSmsForCSV(item.text);
        const whatsappLink = `https://wa.me/91${item.mobile.replace(/\D/g, '')}`;
        
        csv += `"${item.mobile}","${singleLineText}","${whatsappLink}",""\n`;
    });
    
    downloadCSV(csv, `simple_whatsapp_sheet_${getFormattedDate()}.csv`);
    
    // Show simple instructions
    alert(`
✅ Simple WhatsApp Sheet Download Ho Gaya!

HOW TO USE:
1. Open in Excel/Google Sheets
2. Column A: Mobile Numbers
3. Column B: SMS Text
4. Column C: WhatsApp Links
   - Click any link to open WhatsApp chat
   - Message will be pre-filled
5. Column D: Mark "DELETE" in rows you want to remove manually

TIPS:
- Copy column C links to browser to send WhatsApp
- Filter/sort by mobile numbers
- Delete rows manually in Excel
    `);
}

// 8. Optionally update your existing function to add this option
// You can modify the existing HTML button to offer both options:

/*
<button onclick="showWhatsAppOptions()" style="background:#25D366; color:white; font-weight:bold;">
    📱 WhatsApp Sheet
</button>

<div id="whatsappOptions" style="display:none; position:absolute; background:white; border:1px solid #ccc; padding:10px; z-index:1000;">
    <button onclick="downloadWhatsAppSheet()">Advanced (With Macros)</button>
    <button onclick="downloadSimpleWhatsAppSheet()">Simple (Links Only)</button>
</div>
*/
