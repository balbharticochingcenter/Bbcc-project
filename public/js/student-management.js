// ============================================
// STUDENT-MANAGEMENT.JS - CSP COMPLIANT VERSION
// WITH CAMERA CAPTURE & IMAGE COMPRESSION
...
(function() {
    ...
    function capturePhotoFromCamera(elementId) {
        ...
        input.onchange = async (e) => {
            if (e.target.files && e.target.files[0]) {
                try {
                    ...
                    document.getElementById(elementId).value = compressed;
                    // Show preview
                    const previewId = elementId.replace('Photo', 'PhotoPreview').replace('Aadhar', 'AadharPreview');
                    const preview = document.getElementById(previewId);
                    if (preview) {
                        preview.src = compressed;
                        preview.style.display = 'block';
                    }
                    showAlert('Image compressed and added successfully!', 'success');
                } catch (err) {
    ...
    // Upload from file
    function uploadFromFile(elementId) {
        const input = document.createElement('input');
        input.type = 'file';
        input.accept = 'image/*';
        input.onchange = async (e) => {
    ...
    // Default placeholder (data URI - no external request)
    ...
    const styles = `
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .tab-btn {
            flex: 1; min-width: 100px; padding: 15px 20px; background: none;
            border: none; font-size: 1rem; font-weight: 600; color: #666;
        ...
        .btn { padding: 8px 16px; border: none; border-radius: 8px; cursor: pointer; font-size: 0.85rem; transition: all 0.3s; display: inline-flex; align-items: center; gap: 5px; }
        ...
        .modal-content { background: white; border-radius: 20px; max-width: 700px; width: 90%; max-height: 90vh; overflow-y: auto; animation: slideIn 0.3s ease; }
        ...
        .image-preview { width: 80px; height: 80px; border-radius: 10px; object-fit: cover; margin-top: 5px; border: 2px solid #e0e0e0; display: none; }
        .image-actions { display: flex; gap: 10px; margin-top: 5px; flex-wrap: wrap; }
        .image-actions button { flex: 1; font-size: 0.7rem; padding: 5px; }
        ...
        <div class="sms-container">
            ...
            <div class="sms-content">
                ...
                <div class="tab-pane" data-pane="attendance">
                    ...
                    <div class="table-responsive"><table><thead><tr><th>Photo</th><th>ID</th><th>Name</th><th>Board</th><th>Class</th><th>Status</th><th>Check In</th><th>Check Out</th><th>Remarks</th><th>Action</th></tr></thead><tbody id="attendanceTableBody"></tbody></table></div>
                </div>
        ...
        <div id="studentModal" class="modal">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="studentForm">
                        <div class="form-row-3"><div class="form-group"><label>First Name *</label><input type="text" id="firstName" required></div><div class="form-group"><label>Middle Name</label><input type="text" id="middleName"></div><div class="form-group"><label>Last Name *</label><input type="text" id="lastName" required></div></div>
                        <div class="form-row"><div class="form-group"><label>Student ID *</label><input type="text" id="studentId" required placeholder="e.g., STU001"></div><div class="form-group"><label>Password *</label><input type="text" id="password" required></div></div>
                        
                        <!-- Photo Upload Section -->
                        <div class="form-group"><label>Student Photo *</label><input type="hidden" id="photo" required><img id="photoPreview" class="image-preview" alt="Photo Preview"><div class="image-actions"><button type="button" class="btn btn-primary btn-sm" id="capturePhotoBtn">📷 Capture Photo</button><button type="button" class="btn btn-info btn-sm" id="uploadPhotoBtn">📁 Upload Photo</button></div></div>
                        
                        <!-- Aadhar Document Section -->
                        <div class="form-group"><label>Aadhar Document *</label><input type="hidden" id="aadharDocument" required><img id="aadharPreview" class="image-preview" alt="Aadhar Preview"><div class="image-actions"><button type="button" class="btn btn-primary btn-sm" id="captureAadharBtn">📷 Capture Aadhar</button><button type="button" class="btn btn-info btn-sm" id="uploadAadharBtn">📁 Upload Aadhar</button></div></div>
    ...
    class StudentManagementSystem {
        ...
        async init() {
            ...
            if (attendanceDateInput) attendanceDateInput.value = todayDate;
            const reportMonthInput = document.getElementById('reportMonth');
            if (reportMonthInput) reportMonthInput.value = new Date().toISOString().slice(0, 7);
            const joiningDateInput = document.getElementById('joiningDate');
            if (joiningDateInput) joiningDateInput.value = todayDate;
            const promoteJoiningDateInput = document.getElementById('promoteJoiningDate');
            if (promoteJoiningDateInput) promoteJoiningDateInput.value = todayDate;
            const passwordInput = document.getElementById('password');
            if (passwordInput) passwordInput.value = Math.random().toString(36).substring(2, 10);
        ...
        attachEventListeners() {
            ...
            document.getElementById('cancelBlockBtn')?.addEventListener('click', () => closeModal('blockModal'));
            document.getElementById('closePromoteModal')?.addEventListener('click', () => closeModal('promoteModal'));
            document.getElementById('cancelPromoteBtn')?.addEventListener('click', () => closeModal('promoteModal'));
            
            // Image capture/upload buttons
            document.getElementById('capturePhotoBtn')?.addEventListener('click', () => capturePhotoFromCamera('photo'));
            document.getElementById('uploadPhotoBtn')?.addEventListener('click', () => uploadFromFile('photo'));
            document.getElementById('captureAadharBtn')?.addEventListener('click', () => capturePhotoFromCamera('aadharDocument'));
            document.getElementById('uploadAadharBtn')?.addEventListener('click', () => uploadFromFile('aadharDocument'));
            ...
            if (helpBody) {
                const helpItems = [
                    ...
                    { tab: 'attendance', title: '📅 Attendance', desc: 'Mark daily attendance. Filter by board, class. Bulk present option available' },
        ...
        renderRecentIssues(students) {
            ...
            blocked.slice(0, 5).forEach(s => {
                html += `<tr><td>${s.studentName?.first} ${s.studentName?.last}</td><td><span class="badge badge-blocked">${s.accountStatus?.blockReason || 'Unknown'}</span></td><td>${new Date(s.accountStatus?.blockedFrom).toLocaleDateString()}</td><td><button class="btn btn-sm btn-success unblock-btn" data-id="${s.studentId}">Unblock</button></td></tr>`;
            });
        ...
        renderStudentsTable() {
            ...
            tbody.innerHTML = filtered.map(s => `
                <tr>
                    <td><img src="${s.photo || DEFAULT_PHOTO}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;" onerror="this.src='${DEFAULT_PHOTO}'"></td>
                    ...
                    <td>
                        <button class="btn btn-sm btn-info view-student" data-id="${s.studentId}">👁️ View</button>
                        <button class="btn btn-sm btn-warning edit-student" data-id="${s.studentId}">✏️ Edit</button>
                        <button class="btn btn-sm btn-danger block-student" data-id="${s.studentId}" data-name="${s.studentName?.first} ${s.studentName?.last}">🔴 Block</button>
                        ${!s.accountStatus?.isBlocked ? `<button class="btn btn-sm btn-success promote-student" data-id="${s.studentId}" data-name="${s.studentName?.first} ${s.studentName?.last}">📈 Promote</button>` : ''}
                        <button class="btn btn-sm btn-danger delete-student" data-id="${s.studentId}">🗑️ Delete</button>
                    </td>
                </tr>
            `).join('');
        ...
        renderOldStudentsTable() {
            ...
            tbody.innerHTML = filtered.map(s => `
                <tr><td>${s.studentId}</td><td>${s.studentName?.first} ${s.studentName?.last}</td>
                <td>${s.education?.board || '-'}</td><td>${s.education?.class || '-'}</td>
                ...
                <td>${new Date(s.sessionCompletedAt).toLocaleDateString()}</td></tr>
            `).join('');
        }
            ...
            try {
                ...
                if (students.success) {
                    ...
                    document.getElementById('recentRegistrationsBody').innerHTML = recent.map(s => `
                        <tr><td>${new Date(s.registrationDate).toLocaleDateString()}</td><td>${s.studentId}</td>
                        <td>${s.studentName?.first} ${s.studentName?.last}</td><td>${s.education?.board || '-'}</td>
                        <td>${s.education?.class || '-'}</td><td>₹${s.monthlyFees}</td></tr>
                    `).join('');
                }
            } catch (err) { console.error(err); }
        }
        ...
        renderFeesTable() {
            ...
            tbody.innerHTML = feesList.map(f => `
                <tr><td>${f.studentId}</td><td>${f.studentName}</td><td>${f.month} ${f.year}</td>
                <td>₹${f.amount}</td><td>₹${f.paidAmount || 0}</td><td>₹${f.dueAmount || 0}</td>
                <td>${getStatusBadge(f.status)}</td>
                <td><button class="btn btn-sm btn-primary pay-fees" data-id="${f.studentId}" data-month="${f.month}" data-year="${f.year}" data-amount="${f.amount}" data-session="${f.sessionName}">💰 Pay</button></td></tr>
            `).join('');
            ...
            tbody.innerHTML = filtered.map(s => {
                ...
                return `
                    <tr>
                        ...
                        <td><button class="btn btn-sm btn-primary save-attendance" data-id="${s.studentId}" data-date="${selectedDate}">Save</button></td>
                    </tr>
                `;
            }).join('');
        ...
        async editStudent(studentId) {
            ...
            document.getElementById('photo').value = student.photo || DEFAULT_PHOTO;
            document.getElementById('aadharDocument').value = student.aadharDocument || DEFAULT_PHOTO;
            
            // Set previews
            const photoPreview = document.getElementById('photoPreview');
            if (photoPreview) {
                photoPreview.src = student.photo || DEFAULT_PHOTO;
                photoPreview.style.display = 'block';
            }
            const aadharPreview = document.getElementById('aadharPreview');
            if (aadharPreview) {
                aadharPreview.src = student.aadharDocument || DEFAULT_PHOTO;
        ...
        resetStudentForm() {
            ...
            document.getElementById('photo').value = DEFAULT_PHOTO;
            document.getElementById('aadharDocument').value = DEFAULT_PHOTO;
            ...
            const aadharPreview = document.getElementById('aadharPreview');
            if (aadharPreview) {
                aadharPreview.src = DEFAULT_PHOTO;
                aadharPreview.style.display = 'block';
            }
        ...
        async saveStudent() {
            const parentType = document.getElementById('parentType').value;
            const photoValue = document.getElementById('photo').value || DEFAULT_PHOTO;
            const aadharValue = document.getElementById('aadharDocument').value || DEFAULT_PHOTO;
            
            const studentData = {
                studentId: document.getElementById('studentId').value,
                password: document.getElementById('password').value,
                photo: photoValue,
                aadharDocument: aadharValue,
            ...
            try {
                ...
                if (currentEditStudentId) {
                    response = await this.apiCall(`/students/${currentEditStudentId}`, { method: 'PUT', body: JSON.stringify(studentData) });
                } else {
                    response = await this.apiCall('/student-register', { method: 'POST', body: JSON.stringify(studentData) });
                }
                if (response.success) {
        ...
        viewStudent(studentId) {
            ...
            timelineHtml += '</div>';
            showAlert(`<div style="max-height:500px;overflow-y:auto;"><h4>${student.studentName?.first} ${student.studentName?.last}</h4><p><strong>ID:</strong> ${student.studentId}</p><p><strong>Board:</strong> ${student.education?.board} | <strong>Class:</strong> ${student.education?.class}</p><p><strong>Mobile:</strong> ${student.studentMobile}</p><p><strong>Session:</strong> ${student.currentSession?.sessionName} (Ends: ${new Date(student.currentSession?.endDate).toLocaleDateString()})</p><p><strong>Status:</strong> ${student.accountStatus?.isBlocked ? '<span class="badge badge-blocked">Blocked</span>' : '<span class="badge badge-active">Active</span>'}</p><hr>${timelineHtml}</div>`, 'info', 10000);
        }
        ...
        viewStudentDashboard() {
            ...
            const dashboardInfo = document.getElementById('studentDashboardInfo');
            if (dashboardInfo) {
            ...
            const dashboardTimeline = document.getElementById('studentDashboardTimeline');
            if (dashboardTimeline) dashboardTimeline.innerHTML = timelineHtml || '<p>No attendance data</p>';
            let feesHtml = '<div class="table-responsive"><tr><thead><tr><th>Month</th><th>Amount</th><th>Paid</th><th>Due</th><th>Status</th></tr></thead><tbody>';
            ...
            const dashboardFees = document.getElementById('studentDashboardFees');
            if (dashboardFees) dashboardFees.innerHTML = feesHtml || '<p>No fees data</p>';
            ...
            const dashboardBlocks = document.getElementById('studentDashboardBlocks');
            if (dashboardBlocks) dashboardBlocks.innerHTML = blocksHtml || '<p>No block history</p>';
        }
        ...
        generateFeesReport(students, month) {
            ...
            return `<div class="stats-grid"><div class="stat-card"><h3>₹${totalCollected.toLocaleString()}</h3><p>Total Collected</p></div><div class="stat-card"><h3>₹${totalDue.toLocaleString()}</h3><p>Total Due</p></div><div class="stat-card"><h3>${paidCount}</h3><p>Paid</p></div><div class="stat-card"><h3>${partialCount}</h3><p>Partial</p></div><div class="stat-card"><h3>${unpaidCount}</h3><p>Unpaid</p></div></div><div class="table-responsive"><table><thead><tr><th>Student ID</th><th>Name</th><th>Amount</th><th>Paid</th><th>Due</th><th>Status</th></tr></thead><tbody>${reportData.map(r => `<tr><td>${r.id}</td><td>${r.name}</td><td>₹${r.amount}</td><td>₹${r.paid}</td><td>₹${r.due}</td><td>${getStatusBadge(r.status)}</td></tr>`).join('')}</tbody></table></div>`;
        }
        ...
        generateAttendanceReport(students, month) {
            ...
            return `<div class="stats-grid"><div class="stat-card"><h3>${totalPresent}</h3><p>Total Present</p></div><div class="stat-card"><h3>${totalDays}</h3><p>Total Days</p></div><div class="stat-card"><h3>${Math.round(overallPercentage)}%</h3><p>Overall Attendance</p></div></div><div class="table-responsive"><table><thead><tr><th>Student ID</th><th>Name</th><th>Present</th><th>Total Days</th><th>Percentage</th></tr></thead><tbody>${reportData.map(r => `<tr><td>${r.id}</td><td>${r.name}</td><td>${r.present}</td><td>${r.total}</td><td>${Math.round(r.percentage)}%</td></tr>`).join('')}</tbody></table></div>`;
        }
        ...
        generateBlocksReport(students) {
            ...
            return `<div class="stats-grid"><div class="stat-card"><h3>${blockedStudents.length}</h3><p>Currently Blocked</p></div><div class="stat-card"><h3>${blockHistory.length}</h3><p>Total Block Events</p></div></div><div class="table-responsive"><table><thead><tr><th>Student</th><th>Blocked From</th><th>Blocked Until</th><th>Reason</th><th>Status</th></tr></thead><tbody>${blockHistory.map(b => `<tr><td>${b.name} (${b.id})</td><td>${new Date(b.blockedFrom).toLocaleDateString()}</td><td>${b.blockedUntil ? new Date(b.blockedUntil).toLocaleDateString() : 'Permanent'}</td><td>${b.reason}</td><td>${b.unblockedAt ? 'Unblocked' : 'Still Blocked'}</td></tr>`).join('')}</tbody></table></div>`;
        }
        ...
        exportReport() {
            ...
            printWindow.document.write(`<html><head><title>${reportType.toUpperCase()} Report</title><style>body{font-family:Arial;padding:20px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ddd;padding:8px}th{background:#667eea;color:white}.stat-card{background:linear-gradient(135deg,#667eea,#764ba2);color:white;padding:15px;border-radius:10px;margin:10px;display:inline-block;min-width:150px}</style></head><body><h1>${reportType.toUpperCase()} Report - ${month}</h1>${content}<p style="margin-top:20px;">Generated on: ${new Date().toLocaleString()}</p></body></html>`);
            printWindow.print();
        }
    ...
    document.addEventListener('DOMContentLoaded', () => { new StudentManagementSystem(); });
})();
