...
// 🧊 ICE CANDIDATE - FIXED HERE
async function handleIceCandidate(data) {
    const { sender, candidate } = data;  // FIX: Use 'sender' not 'target'
    
    console.log(`🧊 Received ICE from ${sender}`);
    
    if (peerConnections[sender]) {  // FIX: Check 'sender' not 'target'
        const peerConnection = peerConnections[sender].pc;
        try {
            if (candidate) {
                await peerConnection.addIceCandidate(new RTCIceCandidate(candidate));
                console.log(`✅ ICE added for ${sender}`);
            }
        } catch (error) {
